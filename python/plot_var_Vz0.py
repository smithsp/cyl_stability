...
for i in range(1,1000):
  ...
  if match:    
    ...
    g._add_to_queue([Gnuplot.Data([data['Vz0']]*len(evalsr),evalsr)])
#if len(matching_files) != 0:

print Vz0_list, p0_list, eps_list, matching_files
#g('set size 2.0,1.0')
#g('set xrange [-2.1:-1.9]')
g._add_to_queue([Gnuplot.Data([min(Vz0_list),max(Vz0_list)],[cmath.asinh(0).real]*2,with='lines')])
g.title('P1=%g'%(dk['P1']))
set_ytics = 'set ytics ('
for i in range(2):
