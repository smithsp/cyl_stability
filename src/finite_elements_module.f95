...
CONTAINS
  ...
  ELEMENTAL SUBROUTINE bspline_init(this,xj,p1,p2,p3,p4,RendZero,deriv,LendZero)
    ...
    IF((.not.Lend0).and.(.not.Rend0)) THEN
      ...
      !The following are the coefficients for (x-xj)^n, n=0..3
!     denom = 1./((2*dx(2)*dx(3)*dx(1)+2*dx(3)*dx(2)**2+dx(1)*dx(3)**2+&
!      & 2*dx(2)*dx(3)**2+dx(1)*dx(2)*dx(4)+&
!      & dx(2)**2*dx(4)+dx(3)*dx(1)*dx(4)+2*dx(3)*dx(2)*dx(4)))
!     numA = (dx(2)**2+dx(3)**2+dx(3)*dx(4)+dx(2)*dx(4)+&
!      & 2*dx(3)*dx(2))/dx(1)
!     A(0) = 2./3.*(dx(1)+dx(2))**2
!     A(1) = 2.*(dx(1)+dx(2))
!     A(2) = 2.
...
!     A = A*denom*numA
!     B(0) = 2./3./denom
!     B(1) = 2*dx(3)**2+2*dx(3)*dx(4)-2*dx(1)*dx(2)-2*dx(2)**2
!     B(2) = -2*dx(1)-4*dx(2)-4*dx(3)-2*dx(4)
!     B(3) = -2./3.*(dx(1)**2+3*dx(1)*dx(2)+dx(1)*dx(4)+&
!      & 2*dx(3)*dx(1)+4*dx(3)*dx(2)+dx(3)**2+3*dx(2)**2+&
!      & 2*dx(2)*dx(4)+dx(3)*dx(4))/(dx(2)*(dx(1)+dx(2)))
!     B = B*denom
!     C(0:2) = B(0:2)
!     C(3) = 2./3.*(3*dx(3)**2+3*dx(3)*dx(4)+dx(4)**2+2*dx(3)*dx(1)+&
!      & 4*dx(3)*dx(2)+dx(1)*dx(4)+2*dx(2)*dx(4)+&
!      & dx(1)*dx(2)+dx(2)**2)/(dx(3)*(dx(3)+dx(4)))*denom
!     numD = (dx(3)**2+dx(3)*dx(1)+2*dx(3)*dx(2)+dx(1)*dx(2)+&
!      &dx(2)**2)/dx(4)
!     D(0) = 2./3.*(dx(3)+dx(4))**2
!     D(1) = -2*dx(3)-2*dx(4)
!     D(2) = 2.
...
!     D = D*numD*denom
    ELSEIF((this%extent(1).eq.1).and.(this%extent(2).eq.2).and.(Lend0)) THEN
      ...
      ! These are for val(spline,xj-dx[2]) = 0 diff(spline,x,x)@x=xj+dx[3]+dx[4].ne.0
      num2 = 2*(dx(3)+dx(4))**2/dx(2)/(dx(3)+dx(2))/&
      & (2*dx(2)*dx(3)+dx(2)*dx(4)+2*dx(3)**2+2*dx(3)*dx(4))/dx(4)
      B(0) = -(xj-dx(2))**3*num2/0.3D1
      B(1) = (xj-dx(2))**2*num2
      B(2) = (-xj+dx(2))*num2
      B(3) = num2/0.3D1
      
      num3 = 2/dx(3)/(dx(3)+dx(2))/(2*dx(2)*dx(3)+dx(2)*dx(4)+&
      & 2*dx(3)**2+2*dx(3)*dx(4))/dx(4)
      C(0) = (dx(2)**2*dx(3)**3/0.3D1-xj*dx(3)**3*dx(2)+xj**2*dx(3)**3+&
      & 0.2D1/0.3D1*dx(2)**2*dx(3)**2*dx(4)+xj**3*dx(3)**2-0.2D1*xj*dx(3)**2*dx(4)*dx(2)+&
      & 0.2D1*xj**2*dx(3)**2*dx(4)+0.4D1/0.3D1*xj**3*dx(3)*dx(4)-dx(2)*xj*dx(3)*dx(4)**2+&
      & xj**2*dx(3)*dx(4)**2+dx(2)**2*dx(4)**2*dx(3)/0.3D1+xj**3*dx(2)*dx(3)+&
      & xj**3*dx(2)**2/0.3D1+0.2D1/0.3D1*xj**3*dx(2)*dx(4)+xj**3*dx(4)**2/0.3D1)*num3
      C(1) = (-xj**2*dx(2)**2+0.2D1*dx(3)**2*dx(4)*dx(2)-0.3D1*dx(2)*xj**2*dx(3)+&
      & dx(3)**3*dx(2)-0.2D1*dx(2)*xj**2*dx(4)+dx(3)*dx(4)**2*dx(2)-xj**2*dx(4)**2-&
      & 0.4D1*xj*dx(3)**2*dx(4)-0.3D1*xj**2*dx(3)**2-0.2D1*xj*dx(3)*dx(4)**2-&
      & 0.4D1*xj**2*dx(3)*dx(4)-0.2D1*xj*dx(3)**3)*num3
      C(2) = (xj*dx(2)**2+0.3D1*xj*dx(2)*dx(3)+0.2D1*xj*dx(4)*dx(2)+&
      & 0.4D1*xj*dx(3)*dx(4)+0.3D1*xj*dx(3)**2+dx(3)*dx(4)**2+&
      & 0.2D1*dx(3)**2*dx(4)+dx(3)**3+xj*dx(4)**2)*num3
      C(3) = (-dx(2)**2/0.3D1-dx(2)*dx(3)-0.2D1/0.3D1*dx(2)*dx(4)-&
      & dx(4)**2/0.3D1-dx(3)**2-0.4D1/0.3D1*dx(3)*dx(4))*num3
      
      num4 = 2/dx(4)**3/(2*dx(2)*dx(3)+dx(2)*dx(4)+2*dx(3)**2+2*dx(3)*dx(4))
      D(0) = -dble((xj+dx(3)+dx(4))**2*(xj*dx(3)**2+xj*dx(2)*dx(3)+4*xj*dx(3)*dx(4)+&
      & 2*xj*dx(4)*dx(2)+3*xj*dx(4)**2+dx(3)**3+dx(2)*dx(3)**2+&
      & 2*dx(3)**2*dx(4)+dx(3)*dx(4)**2-dx(4)**2*dx(2))*num4)/0.3D1
      D(1) = (xj+dx(3)+dx(4))*(xj*dx(3)**2+xj*dx(2)*dx(3)+4*xj*dx(3)*dx(4)+&
      & 2*xj*dx(4)*dx(2)+3*xj*dx(4)**2+dx(3)**3+dx(2)*dx(3)**2+&
      & 3*dx(3)**2*dx(4)+dx(2)*dx(3)*dx(4)+3*dx(3)*dx(4)**2+dx(4)**3)*num4
      D(2) = (-dx(3)**3-xj*dx(3)**2-dx(2)*dx(3)**2-4*dx(3)**2*dx(4)-&
      & xj*dx(2)*dx(3)-5*dx(3)*dx(4)**2-2*dx(2)*dx(3)*dx(4)-4*xj*dx(3)*dx(4)-&
      & 2*dx(4)**3-3*xj*dx(4)**2-2*xj*dx(4)*dx(2)-dx(4)**2*dx(2))* num4
      D(3) = (dble(dx(3)**2)/0.3D1+dble(dx(2)*dx(3))/0.3D1+&
      & 0.4D1/0.3D1*dble(dx(3))*dble(dx(4))+0.2D1/0.3D1*dble(dx(2))*dble(dx(4))+&
      & dble(dx(4)**2))*dble(num4)

      ! These are for val(spline,xj-dx[2]).ne.0 diff(spline,x)@x=xj-dx[2].eq.0 diff(spline,x,x)@x=xj-dx[2].eq.0
      num2 = 2/dx(2)/(dx(3)+dx(4)+dx(2))/(dx(2)+dx(3))
      B(0) = (0.2D1/0.3D1*dx(2)**2*dx(3)+dx(4)*dx(2)**2/0.3D1+dx(3)**2*dx(2)/0.3D1+&
      & dx(3)*dx(4)*dx(2)/0.3D1-xj**2*dx(2)+dx(2)**2*xj+xj**3/0.3D1)*num2
      B(1) = -(xj-dx(2))**2*num2
      B(2) = (xj-dx(2))*num2
      B(3) = -num2/0.3D1

      num3 = 2/dx(3)/(dx(2)+dx(3))/(dx(3)+dx(4))/(dx(3)+dx(4)+dx(2))
      C(0) = (dx(3)**4/0.3D1+0.2D1/0.3D1*dx(3)**3*dx(4)+0.2D1/0.3D1*dx(3)**3*dx(2)+&
      & dx(3)**2*xj*dx(2)+dx(3)**2*dx(4)*dx(2)-dx(3)**2*xj**2+&
      & dx(3)**2*dx(4)**2/0.3D1+dx(3)*dx(2)*dx(4)**2/0.3D1+&
      & dx(3)*xj*dx(2)*dx(4)-0.2D1/0.3D1*dx(3)*xj**3-&
      & dx(3)*xj**2*dx(4)-dx(2)*xj**3/0.3D1-dx(4)*xj**3/0.3D1)*num3
      C(1) = (-dx(3)**2*dx(2)+0.2D1*dx(3)**2*xj+0.2D1*xj**2*dx(3)-&
      & dx(3)*dx(4)*dx(2)+0.2D1*dx(3)*xj*dx(4)+xj**2*dx(2)+xj**2*dx(4))*num3
      C(2) = (-dx(3)**2-0.2D1*xj*dx(3)-dx(3)*dx(4)-xj*dx(2)-xj*dx(4))*num3
      C(3) = (0.2D1/0.3D1*dx(3)+dx(4)/0.3D1+dx(2)/0.3D1)*num3

      num4 = 2/dx(4)/(dx(3)+dx(4))/(dx(3)+dx(4)+dx(2))
      D(0) = (xj+dx(3)+dx(4))**3*num4/0.3D1
      D(1) = -(xj+dx(3)+dx(4))**2*num4
      D(2) = (xj+dx(3)+dx(4))*num4
      D(3) = -num4/0.3D1

      ! These are for val(spline,xj-dx[2]).ne.0 diff(spline,x)@x=xj-dx[2].eq.0 diff(spline,x,x)@x=xj-dx[2].ne.0
      diff2 = 1.
      num2 = 2/dx(2)**2/(dx(3)+dx(4)+dx(2))/(dx(2)+dx(3))/(2*dx(2)*dx(3)+&
      & dx(4)*dx(2)+2*dx(3)**2+2*dx(3)*dx(4))
      B(0) = ((-0.4D1/0.3D1*dx(3)**3*dx(2)**3-0.2D1*dx(2)**3*xj**3-&
      & 0.2D1/0.3D1*dx(2)*dx(4)**2*xj**3-0.14D2/0.3D1*dx(2)**2*xj**3*dx(3)-&
      & 0.7D1/0.3D1*dx(2)**2*dx(4)*xj**3-0.10D2/0.3D1*dx(3)**2*xj**3*dx(2)+&
      & 0.4D1*dx(3)*dx(4)*dx(2)**2*xj**2+0.2D1*dx(3)*dx(2)**3*xj*dx(4)-&
      & 0.2D1*dx(3)**2*dx(2)**3*dx(4)-0.2D1/0.3D1*dx(3)*dx(2)**3*dx(4)**2+&
      & 0.4D1*dx(3)**2*dx(2)**2*xj**2+0.2D1*dx(3)**2*dx(2)**3*xj-&
      & 0.10D2/0.3D1*dx(4)*xj**3*dx(2)*dx(3)+0.8D1*xj**2*dx(2)**3*dx(3)+&
      & 0.4D1*xj**2*dx(2)**3*dx(4)-0.8D1/0.3D1*dx(2)**4*dx(3)*dx(4)-&
      & 0.2D1*dx(2)**4*xj*dx(3)-dx(2)**4*xj*dx(4)-&
      & 0.8D1/0.3D1*dx(2)**4*dx(3)**2-dx(4)**2*dx(2)**4/0.3D1-&
      & dx(3)**2*dx(4)*xj**3-dx(3)*dx(4)**2*xj**3/0.3D1+&
      & dx(2)**2*dx(4)**2*xj**2+0.2D1*dx(2)**2*dx(3)**3*xj-&
      & 0.2D1/0.3D1*dx(3)**3*xj**3+0.4D1*dx(2)**4*xj**2-&
      & 0.4D1/0.3D1*dx(2)**5*dx(3)-0.2D1/0.3D1*dx(2)**5*dx(4)-&
      & 0.2D1*dx(2)**5*xj+0.3D1*dx(2)**2*dx(3)**2*dx(4)*xj+&
      & dx(2)**2*dx(3)*dx(4)**2*xj)*diff2+0.2D1*dx(3)**3*dx(2)**3+&
      & 0.2D1/0.3D1*dx(3)**4*dx(2)**2+0.2D1/0.3D1*dx(2)**2*xj**3*dx(3)+&
      & dx(2)**2*dx(4)*xj**3/0.3D1+0.2D1/0.3D1*dx(4)*xj**3*dx(2)*dx(3)-&
      & 0.2D1*xj**2*dx(2)**3*dx(3)+0.2D1/0.3D1*dx(3)**2*xj**3*dx(2)+&
      & 0.4D1/0.3D1*dx(2)**4*dx(3)*dx(4)+0.2D1*dx(2)**4*xj*dx(3)+&
      & dx(2)**4*xj*dx(4)+0.4D1/0.3D1*dx(2)**4*dx(3)**2+&
      & dx(4)**2*dx(2)**4/0.3D1-0.2D1*dx(3)*dx(4)*dx(2)**2*xj**2+&
      & 0.4D1/0.3D1*dx(3)**3*dx(4)*dx(2)**2+dx(3)*dx(2)**3*dx(4)**2+&
      & 0.2D1*dx(3)*dx(2)**3*xj*dx(4)+0.2D1*dx(3)**2*dx(2)**3*xj+&
      & 0.2D1/0.3D1*dx(3)**2*dx(4)**2*dx(2)**2-xj**2*dx(2)**3*dx(4)+&
      & 0.3D1*dx(3)**2*dx(2)**3*dx(4)-0.2D1*dx(3)**2*dx(2)**2*xj**2)*num2
      B(1) = ((xj-dx(2))*(0.2D1*dx(3)**3*dx(2)+0.3D1*dx(3)**2*dx(4)*dx(2)+&
      & 0.2D1*dx(2)**2*dx(3)**2+dx(3)*dx(2)*dx(4)**2+&
      & 0.2D1*dx(2)**2*dx(3)*dx(4)+0.10D2*dx(3)*xj*dx(2)*dx(4)+&
      & 0.10D2*dx(3)**2*xj*dx(2)+0.14D2*dx(2)**2*xj*dx(3)+&
      & 0.2D1*dx(2)*xj*dx(4)**2+0.3D1*xj*dx(3)**2*dx(4)+&
      & xj*dx(3)*dx(4)**2+0.2D1*xj*dx(3)**3-0.2D1*dx(2)**3*dx(3)+&
      & 0.7D1*xj*dx(2)**2*dx(4)+0.6D1*dx(2)**3*xj-&
      & dx(4)*dx(2)**3-0.2D1*dx(2)**4)*diff2+(xj-dx(2))*&
      & (0.2D1*dx(2)**2*dx(3)**2-0.2D1*dx(2)**2*xj*dx(3)-&
      & 0.2D1*dx(3)*xj*dx(2)*dx(4)-0.2D1*dx(3)**2*xj*dx(2)-&
      & xj*dx(2)**2*dx(4)+0.2D1*dx(2)**2*dx(3)*dx(4)+&
      & dx(4)*dx(2)**3+0.2D1*dx(2)**3*dx(3)))*num2
      B(2) = ((0.8D1*dx(2)**3*dx(3)+0.4D1*dx(4)*dx(2)**3-0.14D2*dx(2)**2*xj*dx(3)+&
      & 0.4D1*dx(2)**4-0.6D1*dx(2)**3*xj+0.4D1*dx(2)**2*dx(3)**2-&
      & 0.3D1*xj*dx(3)**2*dx(4)-0.10D2*dx(3)*xj*dx(2)*dx(4)-&
      & 0.10D2*dx(3)**2*xj*dx(2)+0.4D1*dx(2)**2*dx(3)*dx(4)+&
      & dx(4)**2*dx(2)**2-0.2D1*xj*dx(3)**3-0.2D1*dx(2)*xj*dx(4)**2-&
      & xj*dx(3)*dx(4)**2-0.7D1*xj*dx(2)**2*dx(4))*diff2+&
      & 0.2D1*dx(2)**2*xj*dx(3)+xj*dx(2)**2*dx(4)-&
      & 0.2D1*dx(2)**3*dx(3)-dx(4)*dx(2)**3-0.2D1*dx(2)**2*dx(3)**2+&
      & 0.2D1*dx(3)**2*xj*dx(2)+0.2D1*dx(3)*xj*dx(2)*dx(4)-&
      & 0.2D1*dx(2)**2*dx(3)*dx(4))*num2
      B(3) = ((0.14D2/0.3D1*dx(2)**2*dx(3)+0.10D2/0.3D1*dx(3)*dx(4)*dx(2)+&
      & 0.10D2/0.3D1*dx(3)**2*dx(2)+dx(3)*dx(4)**2/0.3D1+&
      & 0.2D1/0.3D1*dx(4)**2*dx(2)+0.2D1/0.3D1*dx(3)**3+&
      & dx(3)**2*dx(4)+0.7D1/0.3D1*dx(4)*dx(2)**2+&
      & 0.2D1*dx(2)**3)*diff2-0.2D1/0.3D1*dx(2)**2*dx(3)-&
      & 0.2D1/0.3D1*dx(3)*dx(4)*dx(2)-dx(4)*dx(2)**2/0.3D1-&
      & 0.2D1/0.3D1*dx(3)**2*dx(2))*num2

      num3=1/(dx(3)+dx(4)+dx(2))/(2*dx(2)*dx(3)+dx(4)*dx(2)+2*dx(3)**2+&
      & 2*dx(3)*dx(4))/(dx(3)+dx(4))/(dx(2)+dx(3))/dx(3)
      C(0)=(0.8D1/0.3D1*dx(3)**4*dx(2)**2-0.2D1/0.3D1*dx(2)*dx(4)**2*xj**3-&
      & 0.4D1/0.3D1*dx(2)**2*xj**3*dx(3)-0.2D1/0.3D1*dx(2)**2*dx(4)*xj**3-&
      & 0.4D1*dx(3)**2*xj**3*dx(2)-0.2D1*dx(3)*xj**2*dx(4)**2*dx(2)+&
      & 0.4D1*dx(3)**2*dx(2)*xj*dx(4)**2-&
      & 0.6D1*dx(3)**2*dx(4)*dx(2)*xj**2-0.4D1*dx(3)**3*xj**2*dx(2)+&
      & 0.16D2/0.3D1*dx(3)**3*dx(4)*dx(2)**2+0.4D1*dx(3)**4*xj*dx(2)+&
      & 0.10D2/0.3D1*dx(3)**2*dx(4)**2*dx(2)**2+0.10D2*dx(3)**4*dx(4)*dx(2)+&
      & 0.8D1*dx(3)**3*dx(4)**2*dx(2)-0.8D1*dx(3)**3*xj**2*dx(4)+&
      & 0.2D1*dx(3)**2*dx(4)**3*dx(2)-0.4D1*dx(3)**2*xj**2*dx(4)**2+&
      & 0.2D1/0.3D1*dx(3)*dx(2)**2*dx(4)**3-0.4D1*dx(4)*xj**3*dx(2)*dx(3)+&
      & (0.2D1/0.3D1*dx(4)**3*xj**3-0.8D1/0.3D1*dx(3)**3*dx(2)**3-&
      & 0.16D2/0.3D1*dx(3)**4*dx(2)**2+0.4D1/0.3D1*dx(2)**3*xj**3+&
      & 0.8D1/0.3D1*dx(2)*dx(4)**2*xj**3+0.20D2/0.3D1*dx(2)**2*xj**3*dx(3)+&
      & 0.10D2/0.3D1*dx(2)**2*dx(4)*xj**3+0.28D2/0.3D1*dx(3)**2*xj**3*dx(2)+&
      & 0.8D1*dx(3)*xj**2*dx(4)**2*dx(2)+0.8D1*dx(3)*dx(4)*dx(2)**2*xj**2+&
      & 0.4D1*dx(3)**2*dx(2)*xj*dx(4)**2+0.24D2*dx(3)**2*dx(4)*dx(2)*xj**2-&
      & 0.4D1*dx(3)*dx(2)**3*xj*dx(4)-0.4D1*dx(3)**2*dx(2)**3*dx(4)+&
      & 0.16D2*dx(3)**3*xj**2*dx(2)-0.32D2/0.3D1*dx(3)**3*dx(4)*dx(2)**2+&
      & 0.4D1*dx(3)**4*xj*dx(2)-0.4D1/0.3D1*dx(3)*dx(2)**3*dx(4)**2+&
      & 0.8D1*dx(3)**2*dx(2)**2*xj**2-0.4D1*dx(3)**2*dx(2)**3*xj-&
      & 0.6D1*dx(3)**2*dx(4)**2*dx(2)**2-0.20D2/0.3D1*dx(3)**4*dx(4)*dx(2)-&
      & 0.16D2/0.3D1*dx(3)**3*dx(4)**2*dx(2)+0.16D2*dx(3)**3*xj**2*dx(4)-&
      & 0.4D1/0.3D1*dx(3)**2*dx(4)**3*dx(2)+0.10D2*dx(3)**2*xj**2*dx(4)**2-&
      & 0.2D1/0.3D1*dx(3)*dx(2)**2*dx(4)**3+0.28D2/0.3D1*dx(4)*xj**3*dx(2)*dx(3)+&
      & 0.8D1*dx(3)**3*dx(4)*dx(2)*xj+0.6D1*dx(3)**2*dx(4)*xj**3+&
      & 0.10D2/0.3D1*dx(3)*dx(4)**2*xj**3-0.4D1*dx(2)**2*dx(3)**3*xj-&
      & 0.8D1/0.3D1*dx(3)**5*dx(2)+0.8D1*dx(3)**4*xj**2+&
      & 0.4D1*xj*dx(3)**5+0.10D2*xj*dx(3)**4*dx(4)+&
      & 0.8D1*xj*dx(3)**3*dx(4)**2+0.2D1*xj*dx(3)**2*dx(4)**3+&
      & 0.2D1*xj**2*dx(4)**3*dx(3)+0.4D1*dx(3)**3*xj**3-&
      & 0.6D1*dx(2)**2*dx(3)**2*dx(4)*xj-&
      & 0.2D1*dx(2)**2*dx(3)*dx(4)**2*xj)*diff2+&
      & 0.4D1/0.3D1*dx(3)**6+0.8D1*dx(3)**3*dx(4)*dx(2)*xj-&
      & 0.4D1*dx(3)**2*dx(4)*xj**3-0.4D1/0.3D1*dx(3)*dx(4)**2*xj**3+&
      & 0.4D1*dx(2)**2*dx(3)**3*xj+0.4D1*dx(3)**5*dx(2)+&
      & 0.4D1*dx(3)**5*dx(4)-0.4D1*dx(3)**4*xj**2+&
      & 0.4D1*dx(3)**4*dx(4)**2+0.4D1/0.3D1*dx(3)**3*dx(4)**3-&
      & 0.8D1/0.3D1*dx(3)**3*xj**3+0.6D1*dx(2)**2*dx(3)**2*dx(4)*xj+&
      & 0.2D1*dx(2)**2*dx(3)*dx(4)**2*xj)*num3
      C(1)=((-0.20D2*dx(2)**2*dx(3)*xj**2-0.10D2*dx(4)*dx(2)**2*xj**2-&
      & 0.32D2*dx(3)**3*xj*dx(2)-0.28D2*xj**2*dx(3)*dx(4)*dx(2)-&
      & 0.16D2*dx(2)*dx(3)*xj*dx(4)**2-0.16D2*dx(3)**2*dx(2)**2*xj-&
      & 0.16D2*dx(3)*dx(2)**2*dx(4)*xj-0.2D1*xj**2*dx(4)**3+&
      & 0.4D1*dx(3)*dx(2)**3*dx(4)-0.4D1*dx(3)*dx(4)**3*xj-&
      & 0.2D1*dx(3)**2*dx(4)**3+0.4D1*dx(3)**2*dx(2)**3-&
      & 0.4D1*dx(3)**4*dx(2)-0.32D2*dx(3)**3*xj*dx(4)-&
      & 0.16D2*dx(3)**4*xj+0.4D1*dx(3)**3*dx(2)**2-&
      & 0.12D2*dx(3)**3*xj**2-0.28D2*xj**2*dx(3)**2*dx(2)-&
      & 0.20D2*dx(3)**2*xj*dx(4)**2+0.6D1*dx(3)**2*dx(4)*dx(2)**2-&
      & 0.4D1*dx(3)**2*dx(4)**2*dx(2)-0.18D2*dx(3)**2*xj**2*dx(4)+&
      & 0.2D1*dx(3)*dx(4)**2*dx(2)**2-0.48D2*dx(3)**2*xj*dx(2)*dx(4)-&
      & 0.8D1*dx(3)**3*dx(4)*dx(2)-0.10D2*dx(3)*xj**2*dx(4)**2-&
      & 0.10D2*dx(3)**4*dx(4)-0.8D1*dx(3)**3*dx(4)**2-&
      & 0.8D1*xj**2*dx(4)**2*dx(2)-0.4D1*dx(3)**5-&
      & 0.4D1*xj**2*dx(2)**3)*diff2+0.2D1*dx(4)*dx(2)**2*xj**2+&
      & 0.12D2*xj**2*dx(3)**2*dx(2)+0.16D2*dx(3)**3*xj*dx(4)+&
      & 0.4D1*dx(2)**2*dx(3)*xj**2+0.8D1*dx(3)**3*xj*dx(2)+&
      & 0.12D2*xj**2*dx(3)*dx(4)*dx(2)+0.8D1*dx(3)**4*xj+&
      & 0.2D1*xj**2*dx(4)**2*dx(2)+0.4D1*dx(3)*xj**2*dx(4)**2-&
      & 0.6D1*dx(3)**2*dx(4)*dx(2)**2+0.4D1*dx(2)*dx(3)*xj*dx(4)**2-&
      & 0.4D1*dx(3)**4*dx(2)+0.8D1*dx(3)**2*xj*dx(4)**2+&
      & 0.8D1*dx(3)**3*xj**2-0.2D1*dx(3)*dx(4)**2*dx(2)**2-&
      & 0.4D1*dx(3)**2*dx(4)**2*dx(2)-0.4D1*dx(3)**3*dx(2)**2+&
      & 0.12D2*dx(3)**2*xj**2*dx(4)+0.12D2*dx(3)**2*xj*dx(2)*dx(4)-&
      & 0.8D1*dx(3)**3*dx(4)*dx(2))*num3
      C(2)=((0.4D1*dx(2)**3*xj+0.10D2*xj*dx(2)**2*dx(4)+&
      & 0.2D1*dx(4)**3*dx(3)+0.2D1*xj*dx(4)**3+0.8D1*dx(2)*xj*dx(4)**2+&
      & 0.8D1*dx(3)**4+0.12D2*xj*dx(3)**3+0.16D2*dx(3)**3*dx(2)+&
      & 0.24D2*dx(3)**2*dx(4)*dx(2)+0.8D1*dx(2)**2*dx(3)**2+&
      & 0.18D2*xj*dx(3)**2*dx(4)+0.10D2*xj*dx(3)*dx(4)**2+&
      & 0.16D2*dx(3)**3*dx(4)+0.10D2*dx(3)**2*dx(4)**2+&
      & 0.8D1*dx(3)*dx(2)*dx(4)**2+0.8D1*dx(2)**2*dx(3)*dx(4)+&
      & 0.28D2*dx(3)**2*xj*dx(2)+0.28D2*dx(3)*xj*dx(2)*dx(4)+&
      & 0.20D2*dx(2)**2*xj*dx(3))*diff2-0.8D1*xj*dx(3)**3-&
      & 0.4D1*dx(2)**2*xj*dx(3)-0.12D2*xj*dx(3)**2*dx(4)-&
      & 0.4D1*xj*dx(3)*dx(4)**2-0.4D1*dx(3)**2*dx(4)**2-&
      & 0.4D1*dx(3)**4-0.12D2*dx(3)*xj*dx(2)*dx(4)-&
      & 0.12D2*dx(3)**2*xj*dx(2)-0.6D1*dx(3)**2*dx(4)*dx(2)-&
      & 0.2D1*dx(3)*dx(2)*dx(4)**2-0.2D1*xj*dx(2)**2*dx(4)-&
      & 0.2D1*dx(2)*xj*dx(4)**2-0.8D1*dx(3)**3*dx(4)-&
      & 0.4D1*dx(3)**3*dx(2))*num3
      C(3)=((-0.10D2/0.3D1*dx(4)*dx(2)**2-&
      & 0.8D1/0.3D1*dx(4)**2*dx(2)-0.6D1*dx(3)**2*dx(4)-&
      & 0.20D2/0.3D1*dx(2)**2*dx(3)-0.2D1/0.3D1*dx(4)**3-&
      & 0.28D2/0.3D1*dx(3)*dx(4)*dx(2)-0.10D2/0.3D1*dx(3)*dx(4)**2-&
      & 0.4D1/0.3D1*dx(2)**3-0.28D2/0.3D1*dx(3)**2*dx(2)-&
      & 0.4D1*dx(3)**3)*diff2+&
      & 0.4D1*dx(3)*dx(4)*dx(2)+0.2D1/0.3D1*dx(4)*dx(2)**2+&
      & 0.2D1/0.3D1*dx(4)**2*dx(2)+0.4D1/0.3D1*dx(3)*dx(4)**2+&
      & 0.4D1/0.3D1*dx(2)**2*dx(3)+0.4D1*dx(3)**2*dx(2)+&
      & 0.8D1/0.3D1*dx(3)**3+0.4D1*dx(3)**2*dx(4))*num3      

      num4 = 2*(-dx(4)*dx(2)-2*dx(3)*dx(4)+diff2*dx(4)*dx(2)-&
      & 2*dx(2)*dx(3)+4*diff2*dx(2)*dx(3)+2*diff2*dx(3)**2-&
      & 2*dx(3)**2+diff2*dx(3)*dx(4)+2*diff2*dx(2)**2)/&
      & dx(4)/(dx(3)+dx(4))/(dx(3)+dx(4)+dx(2))/&
      & (2*dx(2)*dx(3)+dx(4)*dx(2)+2*dx(3)**2+&
      & 2*dx(3)*dx(4))
      D(0) = -(xj+dx(3)+dx(4))**3*num4/0.3D1
      D(1) = (xj+dx(3)+dx(4))**2*num4
      D(2) = (-dx(3)-xj-dx(4))*num4
      D(3) = num4/0.3D1

      ! These are for val(spline,xj-dx[2]).ne.0 diff(spline,x)@x=xj-dx[2].eq.0 diff(spline,x,x)@x=xj-dx[2].eq.1
      
      num2 = 0.1D1/dx(2)/(dx(3)+dx(4)+dx(2))/(dx(2)+dx(3))
      B(0) = (-dx(2)**3*dx(3)*dx(4)/0.3D1-dx(3)*dx(4)*xj**3/0.6D1+&
      & dx(3)**2*xj*dx(2)**2/0.2D1+dx(2)**3*xj**2+dx(3)*xj*dx(2)**2*dx(4)/0.2D1+&
      & 0.4D1/0.3D1*dx(2)**2*dx(3)+0.2D1/0.3D1*dx(4)*dx(2)**2+dx(2)**2*dx(3)*xj**2-&
      & 0.2D1/0.3D1*dx(3)*xj**3*dx(2)+0.2D1/0.3D1*dx(3)*dx(4)*dx(2)+&
      & dx(4)*dx(2)**2*xj**2/0.2D1-dx(2)**2*xj**3/0.2D1+0.2D1*dx(2)**2*xj-&
      & 0.2D1*xj**2*dx(2)-dx(2)**4*xj/0.2D1+0.2D1/0.3D1*dx(3)**2*dx(2)-&
      & dx(2)**4*dx(4)/0.6D1-dx(2)**4*dx(3)/0.3D1+0.2D1/0.3D1*xj**3-&
      & dx(3)**2*xj**3/0.6D1-dx(4)*xj**3*dx(2)/0.3D1-dx(2)**3*dx(3)**2/0.3D1)*num2
      B(1) = (xj-dx(2))*(0.2D1*dx(4)*dx(2)*xj+0.3D1*dx(2)**2*xj+&
      & dx(3)*xj*dx(4)+0.4D1*dx(2)*dx(3)*xj+dx(3)**2*xj-&
      & 0.4D1*xj-dx(2)**3+0.4D1*dx(2)+dx(3)*dx(4)*dx(2)+&
      & dx(3)**2*dx(2))*num2/0.2D1
      B(2) = (-0.2D1*dx(2)-dx(3)*xj*dx(4)/0.2D1-dx(4)*dx(2)*xj-&
      & 0.3D1/0.2D1*dx(2)**2*xj-0.2D1*dx(2)*dx(3)*xj+0.2D1*xj+&
      & dx(2)**2*dx(3)+dx(4)*dx(2)**2/0.2D1-dx(3)**2*xj/0.2D1+dx(2)**3)*num2
      B(3) = (-0.2D1/0.3D1+0.2D1/0.3D1*dx(2)*dx(3)+&
      & dx(4)*dx(2)/0.3D1+dx(2)**2/0.2D1+dx(3)**2/0.6D1+dx(3)*dx(4)/0.6D1)*num2

      num3 = 0.1D1/dx(3)/(dx(2)+dx(3))/(dx(3)+dx(4))/(dx(3)+dx(4)+dx(2))
      C(0) = (-0.4D1/0.3D1*dx(3)*xj**3-dx(3)**3*dx(2)**3/0.3D1-&
      & dx(3)**4*dx(2)**2/0.3D1+dx(2)**3*xj**3/0.6D1+&
      & dx(2)*dx(4)**2*xj**3/0.6D1+0.2D1/0.3D1*dx(2)**2*xj**3*dx(3)+&
      & dx(2)**2*dx(4)*xj**3/0.3D1+dx(3)**2*xj**3*dx(2)/0.2D1+&
      & dx(3)*xj**2*dx(4)**2*dx(2)/0.2D1+dx(3)*dx(4)*dx(2)**2*xj**2+&
      & dx(3)**2*dx(2)*xj*dx(4)**2/0.2D1+0.3D1/0.2D1*dx(3)**2*dx(4)*dx(2)*xj**2-&
      & dx(3)*dx(2)**3*xj*dx(4)/0.2D1+0.2D1*dx(3)*xj*dx(2)*dx(4)+&
      & 0.2D1/0.3D1*dx(3)**4+0.2D1*dx(3)**2*xj*dx(2)+0.2D1*dx(3)**2*dx(4)*dx(2)+&
      & 0.2D1/0.3D1*dx(3)*dx(2)*dx(4)**2-0.2D1*dx(3)*xj**2*dx(4)-&
      & dx(3)**2*dx(2)**3*dx(4)/0.2D1-0.2D1/0.3D1*dx(2)*xj**3+&
      & dx(3)**3*xj**2*dx(2)-0.2D1/0.3D1*dx(3)**3*dx(4)*dx(2)**2+&
      & dx(3)**4*xj*dx(2)/0.2D1-dx(3)*dx(2)**3*dx(4)**2/0.6D1+&
      & dx(3)**2*dx(2)**2*xj**2-dx(3)**2*dx(2)**3*xj/0.2D1-&
      & dx(3)**2*dx(4)**2*dx(2)**2/0.3D1+0.2D1/0.3D1*dx(3)**2*dx(4)**2-&
      & 0.2D1*dx(3)**2*xj**2+0.4D1/0.3D1*dx(3)**3*dx(2)+&
      & 0.4D1/0.3D1*dx(3)**3*dx(4)-0.2D1/0.3D1*dx(4)*xj**3+&
      & dx(4)*xj**3*dx(2)*dx(3)/0.2D1+dx(3)**3*dx(4)*dx(2)*xj)*num3
      C(1) = (-dx(3)**4*dx(2)/0.2D1-0.2D1*dx(3)**3*xj*dx(2)-&
      & dx(3)**3*dx(4)*dx(2)+0.4D1*dx(3)**2*xj-0.2D1*dx(3)**2*dx(2)+&
      & dx(2)**3*dx(3)**2/0.2D1-0.3D1/0.2D1*xj**2*dx(3)**2*dx(2)-&
      & 0.3D1*dx(3)**2*xj*dx(2)*dx(4)-0.2D1*dx(3)**2*xj*dx(2)**2-&
      & dx(3)**2*dx(4)**2*dx(2)/0.2D1+dx(2)**3*dx(3)*dx(4)/0.2D1-&
      & 0.2D1*dx(2)**2*dx(3)*xj**2+0.4D1*xj**2*dx(3)-&
      & 0.2D1*dx(3)*dx(4)*dx(2)+0.4D1*dx(3)*xj*dx(4)-&
      & 0.2D1*dx(3)*xj*dx(2)**2*dx(4)-dx(2)*dx(3)*xj*dx(4)**2-&
      & 0.3D1/0.2D1*xj**2*dx(3)*dx(4)*dx(2)-dx(2)**3*xj**2/0.2D1-&
      & dx(4)*dx(2)**2*xj**2+0.2D1*xj**2*dx(2)-&
      & xj**2*dx(4)**2*dx(2)/0.2D1+0.2D1*xj**2*dx(4))*num3
      C(2) = (dx(3)**3*dx(2)+0.3D1/0.2D1*dx(3)**2*xj*dx(2)-&
      & 0.2D1*dx(3)**2+0.3D1/0.2D1*dx(3)**2*dx(4)*dx(2)+&
      & dx(2)**2*dx(3)**2+0.3D1/0.2D1*dx(3)*xj*dx(2)*dx(4)-&
      & 0.4D1*xj*dx(3)+dx(2)**2*dx(3)*dx(4)-0.2D1*dx(3)*dx(4)+&
      & dx(3)*dx(2)*dx(4)**2/0.2D1+0.2D1*dx(2)**2*xj*dx(3)+&
      & xj*dx(2)**2*dx(4)+dx(2)**3*xj/0.2D1-0.2D1*xj*dx(4)-&
      & 0.2D1*xj*dx(2)+dx(2)*xj*dx(4)**2/0.2D1)*num3
      C(3) = (0.4D1/0.3D1*dx(3)+0.2D1/0.3D1*dx(4)+0.2D1/0.3D1*dx(2)-&
      & dx(3)**2*dx(2)/0.2D1-dx(3)*dx(4)*dx(2)/0.2D1-&
      & 0.2D1/0.3D1*dx(2)**2*dx(3)-dx(2)**3/0.6D1-&
      & dx(4)**2*dx(2)/0.6D1-dx(4)*dx(2)**2/0.3D1)*num3

      num4 = dble(1/dx(4)/(dx(3)+dx(4))/(dx(3)+dx(4)+dx(2))*&
      & (-4+dx(2)**2+dx(2)*dx(3)))/0.2D1
      D(0) = -(xj+dx(3)+dx(4))**3*num4/0.3D1
      D(1) = (xj+dx(3)+dx(4))**2*num4
      D(2) = (-dx(3)-xj-dx(4))*num4
      D(3) = num4/0.3D1
      ...
      D(2) = (xj+dx(3)+dx(4))*num4
      D(3) = -num4/0.3D1
      ! These are the coefficients for (x-xj)^n, n=0..3
!     A = 0.
!     B(0) = 0.2D1 / 0.3D1
!     denom1 = (2*dx(3)**2+2*dx(3)*dx(4)+2*dx(3)*dx(2)+dx(2)*dx(4))*dx(2)
!     B(1) = (2*dx(3)**2+2*dx(3)*dx(4)-2*dx(2)**2)/denom1
!     B(2) = (-4*dx(3)-2*dx(4)-4*dx(2))/denom1
!     B(3) = (-0.2D1/0.3D1/(dx(2)**2)*(dx(3)**2+dx(3)*dx(4)+4*dx(3)*dx(2)+2*dx(2)*dx(4)+3*dx(2)**2))/denom1
!     C(0:2) = B(0:2)
!     C(3) = 0.2D1/0.3D1/(dx(3))*(dx(2)**2+2*dx(2)*dx(4)+4*dx(3)*dx(2)+3*dx(3)**2+3*dx(3)*dx(4)+dx(4)**2)/&
!     & (2*dx(3)**3+2*dx(2)*dx(3)**2+4*dx(3)**2*dx(4)+3*dx(3)*dx(2)*dx(4)+2*dx(3)*dx(4)**2+dx(4)**2*dx(2))/(dx(2))
!     denom1 =   dx(2)*(2*dx(3)**2+2*dx(3)*dx(4)+2*dx(3)*dx(2)+dx(2)*dx(4))*dx(4)/(dx(3)+dx(2))**2
!     D(0) = (0.2D1/0.3D1*dx(3)**2+0.4D1/0.3D1*dx(3)*dx(4)+0.2D1/0.3D1*dx(4)**2)/denom1
!     D(1) = (-0.2D1*dx(3)-0.2D1*dx(4))/denom1
!     D(2) = 2.0/denom1
!     D(3) = -0.2D1/0.3D1*((dx(3)+dx(2))**2)/(dx(2))/(dx(4))/&
!     & (2*dx(3)**3+2*dx(2)*dx(3)**2+4*dx(3)**2*dx(4)+3*dx(3)*dx(2)*dx(4)+2*dx(3)*dx(4)**2+dx(4)**2*dx(2))
    ELSEIF((this%extent(1).eq.2).and.(this%extent(2).eq.1).and.Rend0) THEN
      ...
      !These are for val(spline,x=xj+dx[3])=0, diff(spline,x)@x=xj+dx[3].ne.0, diff(spline,x)@x=xj+dx[3].eq.0
      num1 = 2*(2*dx(3)+dx(2))*(dx(3)+dx(2))/dx(1)/(dx(1)+dx(2))/dx(3)/(2*dx(3)*dx(1)+&
      & 4*dx(3)*dx(2)+3*dx(1)*dx(2)+3*dx(2)**2)
      A(0) = (-xj+dx(1)+dx(2))**3*num1/0.3D1
      A(1) = (-xj+dx(1)+dx(2))**2*num1
      A(2) = (-xj+dx(1)+dx(2))*num1
      A(3) = num1/0.3D1
      
      num2 = 2/dx(2)/(dx(1)+dx(2))/dx(3)/(2*dx(3)*dx(1)+&
      & 4*dx(3)*dx(2)+3*dx(1)*dx(2)+3*dx(2)**2)
      B(0) = (-dble(3*dx(2)**2*xj**2*dx(1))-&
      & dble(2*dx(2)**3*xj**2)-dble(dx(2)*xj**2*dx(1)**2)+&
      & dble(dx(1)**2*xj**3)/0.3D1+dble(dx(1)**2*dx(2)**2*xj)+&
      & dble(2*dx(1)*dx(2)**3*xj)-&
      & dble(3*dx(3)*dx(2)*xj**2*dx(1))-dble(3*dx(3)*dx(2)**2*xj**2)+&
      & dble(dx(3)*dx(1)*xj**3)+&
      & dble(dx(2)**2*xj**3)+dble(dx(1)*dx(2)*xj**3)+&
      & 0.2D1/0.3D1*dble(dx(1)**2)*dble(dx(2))*dble(dx(3)**2)+&
      & dble(2*dx(1)*dx(2)**2*dx(3)**2)+dble(dx(1)**2*dx(3)*dx(2)**2)+&
      & dble(2*dx(1)*dx(2)**3*dx(3))+&
      & 0.4D1/0.3D1*dble(dx(2)**3)*dble(dx(3)**2)+dble(dx(2)**4*dx(3))+&
      & dble(2*dx(3)*dx(2)*xj**3)-&
      & dble(2*dx(2)*dx(3)**2*xj*dx(1))-dble(2*dx(2)**2*dx(3)**2*xj)+&
      & 0.2D1/0.3D1*dble(dx(3)**2)*dble(xj**3)+&
      & dble(dx(2)**4*xj))*num2
      B(1) = dble(-dx(1)**2*dx(2)**2-2*dx(1)*dx(2)**3+&
      & 6*dx(2)**2*xj*dx(1)+4*dx(2)**3*xj+&
      & 2*dx(3)**2*dx(2)*dx(1)+2*dx(3)**2*dx(2)**2+&
      & 2*dx(2)*xj*dx(1)**2-dx(1)**2*xj**2-&
      & 3*xj**2*dx(1)*dx(2)+6*xj*dx(2)*dx(3)*dx(1)+&
      & 6*xj*dx(2)**2*dx(3)-dx(2)**4-&
      & 3*xj**2*dx(1)*dx(3)-3*dx(2)**2*xj**2-&
      & 2*dx(3)**2*xj**2-6*xj**2*dx(2)*dx(3))*num2
      B(2) = dble(3*xj*dx(3)*dx(1)-dx(1)**2*dx(2)-&
      & 3*dx(1)*dx(2)**2+3*dx(2)*xj*dx(1)+xj*dx(1)**2+&
      & 6*xj*dx(2)*dx(3)+3*dx(2)**2*xj-2*dx(2)**3+2*dx(3)**2*xj-&
      & 3*dx(3)*dx(2)*dx(1)-3*dx(3)*dx(2)**2)*num2
      B(3) = (-dble(dx(1)**2)/0.3D1-dble(dx(3)*dx(1))-&
      & dble(dx(1)*dx(2))-dble(2*dx(3)*dx(2))-&
      & dble(dx(2)**2)-0.2D1/0.3D1*dble(dx(3)**2))*num2

      num3 = 2/dx(3)**2/(2*dx(3)*dx(1)+4*dx(3)*dx(2)+3*dx(1)*dx(2)+3*dx(2)**2)
      C(0) = dble((xj+dx(3))*(-2*dx(2)*xj**2-3*dx(3)*xj**2-dx(1)*xj**2-2*xj*dx(3)*dx(1)-&
      & 6*dx(3)**2*xj-4*xj*dx(2)*dx(3)+3*dx(3)*dx(2)**2+3*dx(3)*dx(2)*dx(1)+2*dx(3)**2*dx(1)+&
      & 4*dx(3)**2*dx(2))*num3)/0.3D1
      C(1) = (-dx(3)*dx(2)**2-dx(3)*dx(2)*dx(1)+3*dx(3)*xj**2+2*dx(3)**3+&
      & 6*dx(3)**2*xj+4*xj*dx(2)*dx(3)+dx(1)*xj**2+2*xj*dx(3)*dx(1)+2*dx(2)*xj**2)*num3
      C(2) = -(xj+dx(3))*(3*dx(3)+dx(1)+2*dx(2))*num3
      C(3) = (dble(dx(3))+dble(dx(1))/0.3D1+0.2D1/0.3D1*dble(dx(2)))*dble(num3)
      D = 0.
      
      !These are for val(spline,x=xj+dx[3])=0, diff(spline,x)@x=xj+dx[3].eq.0, diff(spline,x)@x=xj+dx[3].ne.0
            
      num1 = 2*(xj-dx(2)-dx(1))**2*(dx(3)+dx(2))**2/dx(1)/(dx(2)+dx(1))/&
      & (dx(1)*dx(3)+2*dx(2)*dx(1)+2*dx(2)**2+2*dx(2)*dx(3))/dx(3)
      A(0) = (-xj/0.3D1+dx(2)/0.3D1+dx(1)/0.3D1)*num1
      A(1) = num1
      A(2) = -0.1D1/(xj-dx(2)-dx(1))*num1
      A(3) = 0.1D1/(xj-dx(2)-dx(1))**2*num1/0.3D1
      ...
      ! These are the coefficients for (x-xj)^n, n=0..3
!     denom = (dx(3)*dx(1)+2*dx(1)*dx(2)+2*(dx(2))**2+2*dx(3)*dx(2))*dx(3)*dx(1)
!     numA = 2*(dx(3)+dx(2))**2/denom
!     A(0) = 1./3.*(dx(1)+dx(2))**2
!     A(1) = dx(1)+dx(2)
!     A(2) = 1.
!     A(3) = 1./(3.*(dx(1)+dx(2)))
...
!     denom = denom/dx(1)
!     B(0) = 2./3.
!     B(1) = -2*(dx(1)*dx(2)+(dx(3))**2-(dx(2))**2)
!     B(2) = -2*(2*dx(3)+dx(1)+2*dx(2))
!     B(3) = -2./3.*(dx(1)**2+3*dx(1)*dx(2)+2*dx(3)*dx(1)+&
!      & 4*dx(3)*dx(2)+dx(3)**2+3*dx(2)**2)/(dx(2)*(dx(1)+dx(2)))
!     B(1:3) = B(1:3)/denom
!     C(0:2) = B(0:2)
!     C(3) = 2./3.*(3.*dx(3)**2+2.*dx(3)*dx(1)+4.*dx(3)*dx(2)+dx(1)*dx(2)+dx(2)**2)/dx(3)**2/denom
!     D = 0.
    ELSEIF((this%extent(1).eq.0).and.(this%extent(2).eq.2).and.(Lend0)) THEN !.and.(.not.this%deriv)
      ...
      ! These are for val(spline,xj)=0, diff(spline,x)@x=xj=0, diff(spline,x,x)@x=xj,xj+dx[3].ne.0
      num3 = 2/dx(4)/dx(3)**3
      C(0) = dble((3*dx(3)**2+3*xj*dx(3)+3*dx(4)*dx(3)+2*xj*dx(4))*xj**2*num3)/0.3D1
      C(1) = -(2*dx(3)**2+2*dx(4)*dx(3)+3*xj*dx(3)+2*xj*dx(4))*xj*num3
      C(2) = (dx(3)**2+dx(4)*dx(3)+3*xj*dx(3)+2*xj*dx(4))*num3
      C(3) = (-dble(dx(3))-0.2D1/0.3D1*dble(dx(4)))*dble(num3)
...
!     num3 = 2/dx(3)**3/dx(4)**2
!     C(0) = -dble((3*dx(3)**4+6*xj*dx(3)**3+6*dx(4)*dx(3)**3+9*xj*dx(4)*dx(3)**2+3*dx(4)**2*dx(3)**2+&
!     & 3*xj**2*dx(3)**2+3*xj*dx(4)**2*dx(3)+3*xj**2*dx(3)*dx(4)+xj**2*dx(4)**2)*xj*num3)/0.3D1
!     C(1) = (dx(3)**4+4*xj*dx(3)**3+2*dx(4)*dx(3)**3+6*xj*dx(4)*dx(3)**2+dx(4)**2*dx(3)**2+3*xj**2*dx(3)**2+&
!     & 2*xj*dx(4)**2*dx(3)+3*xj**2*dx(3)*dx(4)+xj**2*dx(4)**2)*num3
!     C(2) = (-2*dx(3)**3-3*dx(3)**2*dx(4)-3*xj*dx(3)**2-dx(3)*dx(4)**2-3*xj*dx(3)*dx(4)-xj*dx(4)**2)*num3
!     C(3) = (dble(dx(3)**2)+dble(dx(3)*dx(4))+dble(dx(4)**2)/0.3D1)*dble(num3)
!
!     num4 = 2 / dx(4) ** 3
!     D(0) = (xj+dx(3)+dx(4))**3*num4/0.3D1
!     D(1) = -(xj+dx(3)+dx(4))**2*num4
...
!     D(3) = -num4/0.3D1
    ...
    ELSEIF((this%extent(1).eq.0).and.(this%extent(2).eq.1).and.Lend0) THEN
      ...
      num3 = 0.1D1/dx(3)**2
      C(0) = -xj**2*(xj+dx(3))*num3
      C(1) = xj*(2*dx(3)+3*xj)*num3
      C(2) = (-dx(3)-3*xj)*num3
      C(3) = num3
