...
CONTAINS
  ...
  FUNCTION B23(r)
    ...
    B23 = rho(r)*Bt(r)*kV(r)*r
  END FUNCTION B23
  FUNCTION A33(r)
    IMPLICIT NONE
    REAL(r8), INTENT(IN), DIMENSION(:) :: r
    REAL(r8), DIMENSION(size(r)) :: A33
    A33 = rho(r)*(Bz(r)**2+Bt(r)**2)
  END FUNCTION A33
  FUNCTION B33(r)
    IMPLICIT NONE
    REAL(r8), INTENT(IN), DIMENSION(:) :: r
    REAL(r8), DIMENSION(size(r)) :: B33
    B33 = rho(r)*(Bz(r)**2+Bt(r)**2)*kV(r)
  END FUNCTION B33
  FUNCTION B41a(r)
    IMPLICIT NONE
    REAL(r8), INTENT(IN), DIMENSION(:) :: r
    REAL(r8), DIMENSION(size(r)) :: B41a
    B41a = (gamma*P(r)+Bz(r)**2+Bt(r)**2)
  END FUNCTION B41a
  FUNCTION B41b(r)
    IMPLICIT NONE
    REAL(r8), INTENT(IN), DIMENSION(:) :: r
  ...
  FUNCTION B41bb(r)
    ...
    B41bb = (Bz(r)**2+gamma*P(r))/r
  END FUNCTION B41bb
  FUNCTION B41c(r)
    IMPLICIT NONE
    REAL(r8), INTENT(IN), DIMENSION(:) :: r
    REAL(r8), DIMENSION(size(r)) :: B41c
    B41c = kdotB(r)**2+rho(r)*Vp(r)**2-Bt(r)**2/r**2
  END FUNCTION B41c
  FUNCTION B42b(r)
    IMPLICIT NONE
    REAL(r8), INTENT(IN), DIMENSION(:) :: r
    REAL(r8), DIMENSION(size(r)) :: B42b
    B42b = (gamma*P(r)+Bz(r)**2)*mt-Bt(r)*Bz(r)*kz*r
  END FUNCTION B42b
  FUNCTION B42c(r)
    IMPLICIT NONE
    REAL(r8), INTENT(IN), DIMENSION(:) :: r
    REAL(r8), DIMENSION(size(r)) :: B42c
    B42c = Bt(r)*kz*Bz(r)+(Bz(r)**2+gamma*p(r))*mt/r+r*Vp(r)**2*rho(r)*mt
  END FUNCTION B42c
  FUNCTION B43b(r)
    IMPLICIT NONE
    REAL(r8), INTENT(IN), DIMENSION(:) :: r
  ...
  FUNCTION B43c(r)
    ...
    B43c = (rho(r)*Vp(r)**2*r+gamma*p(r)/r)*kdotB(r)
  END FUNCTION B43c
  FUNCTION B51c(r)
    IMPLICIT NONE
    REAL(r8), INTENT(IN), DIMENSION(:) :: r
    REAL(r8), DIMENSION(size(r)) :: B51c
    B51c = 2*Bt(r)*kz*Bz(r)+r*Vp(r)**2*rho(r)*mt
  END FUNCTION B51c
  FUNCTION B52(r)
    IMPLICIT NONE
    REAL(r8), INTENT(IN), DIMENSION(:) :: r
    REAL(r8), DIMENSION(size(r)) :: B52
    B52 = gamma*P(r)*mt**2+rho(r)*r**2*Vp(r)**2+Bz(r)**2*(mt**2+kz**2*r**2)
  END FUNCTION B52
  FUNCTION B53(r)
    IMPLICIT NONE
    REAL(r8), INTENT(IN), DIMENSION(:) :: r
    REAL(r8), DIMENSION(size(r)) :: B53
    B53 = gamma*P(r)*mt*kdotB(r)+Bt(r)*rho(r)*Vp(r)**2*r
  END FUNCTION B53
  FUNCTION B61c(r)
    IMPLICIT NONE
    REAL(r8), INTENT(IN), DIMENSION(:) :: r
    REAL(r8), DIMENSION(size(r)) :: B61c
    B61c = rho(r)*Vp(r)**2*kdotB(r)*r
  END FUNCTION B61c
  FUNCTION B63(r)
    IMPLICIT NONE
    REAL(r8), INTENT(IN), DIMENSION(:) :: r
    REAL(r8), DIMENSION(size(r)) :: B63
    B63 = gamma*P(r)*kdotB(r)**2+rho(r)*Vp(r)**2*Bt(r)**2
  END FUNCTION B63
  ...
  FUNCTION BCB1(r)
    ...
    BCB1 = -Bt(r)**2/r+r*rho(r)*Vp(r)**2
  END FUNCTION BCB1
  FUNCTION BCB1nw(r)
    IMPLICIT NONE
    REAL(r8), INTENT(IN), DIMENSION(:) :: r
    REAL(r8), DIMENSION(size(r)) :: BCB1nw
    BCB1nw = -Ka*kdotB(r)**2/(kz*Kadot)
  END FUNCTION BCB1nw
  FUNCTION BCB1cw(r)
    IMPLICIT NONE
    REAL(r8), INTENT(IN), DIMENSION(:) :: r
    REAL(r8), DIMENSION(size(r)) :: BCB1cw
    BCB1cw = -kdotB(r)**2*(Lbdot*Ka-Kbdot*La)/(kz*(Kadot*Lbdot-Ladot*Kbdot))
  END FUNCTION BCB1cw
  FUNCTION BJCP1(r)
    IMPLICIT NONE
    REAL(r8), INTENT(IN), DIMENSION(:) :: r
