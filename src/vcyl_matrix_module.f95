...
CONTAINS
  ...
  FUNCTION B42b(r)
    ...
    B42b = (gamma*P(r)*mt+Bz(r)**2*mt)/r-Bt(r)*Bz(r)*kz
  END FUNCTION B42b
  FUNCTION B42c(r)
    IMPLICIT NONE
    REAL(r8), INTENT(IN), DIMENSION(:) :: r
    REAL(r8), DIMENSION(size(r)) :: B42c
    B42c = 2*Bt(r)*kz*Bz(r)/r+Vp(r)**2*rho(r)*mt
  END FUNCTION B42c
  FUNCTION B43b(r)
    IMPLICIT NONE
    REAL(r8), INTENT(IN), DIMENSION(:) :: r
  ...
  FUNCTION B43c(r)
    ...
    B43c = rho(r)*Vp(r)**2*kdotB(r)
  END FUNCTION B43c
  FUNCTION B52(r)
    IMPLICIT NONE
    REAL(r8), INTENT(IN), DIMENSION(:) :: r
    REAL(r8), DIMENSION(size(r)) :: B52
    B52 = (gamma*P(r)*mt**2+rho(r)*r**2*Vp(r)**2+Bz(r)**2*(mt**2+kz**2*r**2))/r
  END FUNCTION B52
  FUNCTION B53(r)
    IMPLICIT NONE
    REAL(r8), INTENT(IN), DIMENSION(:) :: r
    REAL(r8), DIMENSION(size(r)) :: B53
    B53 = gamma*P(r)*mt*kdotB(r)/r+Bt(r)*rho(r)*Vp(r)**2
  END FUNCTION B53
  FUNCTION B63(r)
    IMPLICIT NONE
    REAL(r8), INTENT(IN), DIMENSION(:) :: r
    REAL(r8), DIMENSION(size(r)) :: B63
    B63 = gamma*P(r)*kdotB(r)**2/r+rho(r)*Vp(r)**2*Bt(r)**2/r
  END FUNCTION B63
