...
!The following are all used in subroutines below and should not be used in the main (cyl) program
  ...
  Evecs = .false.
  epskVa = 0.
  nu = 0.0  !This is a multiplicative factor on the critical coefficient 
            !of numerical diffusion 
            !added to damp the fastest fast waves
  kappa = 0.0 !This is the parallel viscosity term
  CALL cpu_time(t1)
...
SUBROUTINE linear_const_sa() !sa stands for self adjoint
  ...
  SUBROUTINE linear_const_rw()
    ...
    REAL(r8) :: xi1pNa, xi2pNa, xi1Na, xi2Na, xi3Na, xi1pN1a, xi2pN1a, ABNRM, BBNRM
    REAL(r8), DIMENSION(:), ALLOCATABLE :: LSCALE, RSCALE, RCONDE, RCONDV
    CHARACTER :: balanc, sense
    INTEGER :: lower1, upper1, IHI, ILO, LDA, LDB
    ...
    ENDIF
    IF (nu.ne.0) THEN
      WRITE(0,'(a)') '', 'Warning!!  Viscous surface terms have not been implemented yet.', ''
    ENDIF
    vcyl = .true.
    lower1 = 1
    upper1 = N-1
  ...
  ! We're going to add 1 to NN for the coefficient C2 of the vacuum solution
    ...
    LWORK=10*NN
    ALLOCATE(A(NN,NN),B(NN,NN),C(NN,NN),D(NN,NN),VR(NN,NN),VL(1,NN),ALPHA(NN),BETA(NN),RWORK(LWORK),WORK(10*NN))
    ALLOCATE(LSCALE(NN),RSCALE(NN),RCONDE(NN),RCONDV(NN),IWORK(NN+2),BWORK(NN))
    LDVR = NN
  ! Initialize the grid
    grid = (/ (i*ar/(N-1), i=0,N-1) /)
    grid = new_grid(grid)
  ...
  !k is the row of the submatrix
    ...
    DO i=lower(m),upper(m)
      ...
      DO j=max(i-3,lower(l)),min(i+3,upper(l))
        A(6*(i-lower(m))+k,6*(j-lower(l))+l) = A(6*(i-lower(m))+1,6*(j-lower(l))+1)
        B(6*(i-lower(m))+k,6*(j-lower(l))+l) = B(6*(i-lower(l))+m,6*(j-lower(m))+m)&
          & + nu*h**2*cmplx(0,1,r8)*int_func(phi1(i),phi1(j),B44i,deriv1=.true.,deriv2=.true.)
      ENDDO
      l = 5
      DO j=max(i-3,lower(l)),min(i+3,upper(l))
        A(6*(i-lower(m))+k,6*(j-lower(l))+l) = A(6*(i-lower(m))+1,6*(j-lower(l))+2)
        B(6*(i-lower(m))+k,6*(j-lower(l))+l) = B(6*(i-lower(l))+1,6*(j-lower(m))+2)&
          & + nu*h**2*cmplx(0,1,r8)*int_func(phi1(i),phi2(j),B45i,deriv1=.true.,deriv2=.true.)
      ENDDO
      l = 6
      DO j=max(i-3,lower(l)),min(i+3,upper(l))
        B(6*(i-lower(m))+k,6*(j-lower(l))+l) = B(6*(i-lower(l))+1,6*(j-lower(m))+3)
    ...
    DO i=lower(m),upper(m)
      ...
      DO j=max(i-3,lower(l)),min(i+3,upper(l))
        A(6*(i-lower(m))+k,6*(j-lower(l))+l) = A(6*(i-lower(l))+2,6*(j-lower(m))+1)
        B(6*(i-lower(m))+k,6*(j-lower(l))+l) = B(6*(j-lower(l))+4,6*(i-lower(m))+5)
      ENDDO
      l = 5
      DO j=max(i-3,lower(l)),min(i+3,upper(l))
        A(6*(i-lower(m))+k,6*(j-lower(l))+l) = A(6*(i-lower(l))+2,6*(j-lower(m))+2)
        B(6*(i-lower(m))+k,6*(j-lower(l))+l) = B(6*(i-lower(l))+2,6*(j-lower(m))+2)&
          & + nu*h**2*cmplx(0,1,r8)*int_func(phi2(i),phi2(j),B55i,deriv1=.true.,deriv2=.true.)
      ENDDO
      l = 6
      DO j=max(i-3,lower(l)),min(i+3,upper(l))
        B(6*(i-lower(m))+k,6*(j-lower(l))+l) = B(6*(i-lower(l))+2,6*(j-lower(m))+3)
    ...
    DO i=lower(m),upper(m)
      ...
      DO j=max(i-3,lower(l)),min(i+3,upper(l))
        B(6*(i-lower(m))+k,6*(j-lower(l))+l) = B(6*(i-lower(l))+3,6*(j-lower(m))+1)&
          & + nu*h**2*cmplx(0,1,r8)*int_func(phi3(i),phi1(j),B64i,deriv2=.true.)
      ENDDO
      l = 5
      DO j=max(i-3,lower(l)),min(i+3,upper(l))
        B(6*(i-lower(m))+k,6*(j-lower(l))+l) = B(6*(i-lower(l))+3,6*(j-lower(m))+2)&
          & + nu*h**2*cmplx(0,1,r8)*int_func(phi3(i),phi2(j),B65i,deriv2=.true.)
      ENDDO
      l = 6
      DO j=max(i-3,lower(l)),min(i+3,upper(l))
        A(6*(i-lower(m))+k,6*(j-lower(l))+l) = A(6*(i-lower(l))+3,6*(j-lower(m))+3)
        B(6*(i-lower(m))+k,6*(j-lower(l))+l) = B(6*(i-lower(l))+3,6*(j-lower(m))+3)&
          & + kappa*cmplx(0,1,r8)*int_func(phi3(i),phi3(j),B66i)
      ENDDO
    ENDDO
    ! This is for a wall not at the plasma surface
    IF (br.gt.ar) THEN
    ...
    B=B+D!Sign?    
    A=A+C
    ! This Doppler shifts the boundary condition, 
    ! which is also done in vcyl_matrix_module for the other parts of B,
    ! which is corrected for later
    B(NN,:) = B(NN,:)-kVa*A(NN,:)
  ! This is to account for regularity at the center
    !IF (mt.ne.1) THEN
      IF (lower(1).le.1) THEN
    ...
    CALL cpu_time(st1)
    IF (Evecs) THEN
      CALL ZGGEVX('B','N','V','N',NN,B,NN,A,NN,ALPHA,BETA,VL,LDVL,VR,LDVR,ILO,IHI,LSCALE,RSCALE,ABNRM,BBNRM,RCONDE,RCONDV,WORK,LWORK,RWORK,IWORK,BWORK,INFO)
    ELSE
      CALL ZGGEVX('B','N','N','N',NN,B,NN,A,NN,ALPHA,BETA,VL,LDVL,VR,LDVR,ILO,IHI,LSCALE,RSCALE,ABNRM,BBNRM,RCONDE,RCONDV,WORK,LWORK,RWORK,IWORK,BWORK,INFO)
    ENDIF
    ...
    CALL output_params(at2-at1,st2-st1)
    !---------
    !Here omega is reshifted by kVa to allow the postprocessing routines to remain unchanged
    IF (maxval(abs(AIMAG(BETA))).EQ.0) THEN
      CALL output_evals(REAL(ALPHA)/REAL(BETA)+kVa,AIMAG(ALPHA)/REAL(BETA))
    ELSE
      CALL output_evals(REAL(ALPHA/BETA)+kVa,AIMAG(ALPHA/BETA))
    ENDIF
    !-----------
 ...
 ! There was a k,kz, m,mt mistake in the following line
    ...
    ENDDO
    C(NN,NN) = cmplx(0,1,r8)*kz*br*(Kbdot*Ladot-Kadot*Lbdot)/(mt**2+kz**2*br**2)/Kadot
    D(NN,NN) = -(Kbdot*Lb-Kb*Lbdot)/Kbdot/(tw*br)
    B=B+D
    A=A+C
    ! This is to account for regularity at the center
    !IF (mt.ne.1) THEN
  ...
  SUBROUTINE output_params(assemble_t,solve_t)
    ...
    WRITE(1,'(g)') epsilo, assemble_t, solve_t, kz, ar, br, tw, rho0, Bz0, Bt0, s2, eps, P0, P1, lambd, Vz0, epsVz, Vp0, epsVp, rs, alpha, epskVa, nu
